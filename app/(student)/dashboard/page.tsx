...
export default async function StudentDashboardPage() {
  ...
  // If this account is an admin (no linked student), send them to the admin dashboard
  if (isAdmin(auth)) {
    redirect("/admin");
  }
  ...
  if (!auth.profile || !auth.profile.student_id) {
    return (
      <main className="min-h-screen flex flex-col bg-background-light font-display text-slate-900">
  ...
  const student = await getStudentByIdServer(auth.profile.student_id);
